...
> đâu cũng được. Hai script TTS (`taogiong.py`, `taogiong_gui.py`) tự thêm gốc repo vào
> `sys.path` để `import omnivoice`.

**Chạy** (từ gốc repo `OmniVoice/`):

```
venv\Scripts\python myvoice\scripts\taogiong_gui.py
venv\Scripts\python myvoice\scripts\video_khung.py
```
...
input.txt  ──(taogiong_gui.py / taogiong.py)──►  output.wav + output_chunks/*.wav
                            ...
                            taogiong_kiemtra_audio.py  (kiểm tra chunk lỗi)
                                              │
mp4_no_audio/*.mp4  ──(video_ghepcuoi.py)──►  kịch_bản/final_video.mp4
...
### `taogiong_gui.py` — Giao diện chính (khuyên dùng)
**Chạy:** `python taogiong_gui.py`
...
### `taogiong.py` — Phiên bản dòng lệnh (không GUI)
**Chạy:** `python taogiong.py` (chỉnh phần `CẤU HÌNH` ở đầu file trước)
...
### `taogiong_kiemtra_audio.py` — Kiểm tra chunk audio lỗi
**Chạy:** `python taogiong_kiemtra_audio.py`
...
> Là công cụ chẩn đoán độc lập; `taogiong_gui.py` đã tích hợp sẵn cơ chế tương tự nên thường chỉ cần khi muốn kiểm tra thủ công.

### `video_ghepcuoi.py` — Ghép video cuối + nhạc nền
**Chạy:** `python video_ghepcuoi.py` (cần **ffmpeg/ffprobe** trong PATH)
...
### `video_xoatieng.py` — Xóa tiếng + scale video nguồn
**Chạy:** `python video_xoatieng.py` (cần **ffmpeg** trong PATH)

- Duyệt mọi `.mp4` trong `mp4/`, xóa âm thanh và scale về 1080p dọc (`scale=1080:-2`).
- Lưu sang `mp4_no_audio/` giữ nguyên tên file (đầu vào cho `video_ghepcuoi.py`).

### `video_khung.py` — Lồng video vào bộ khung trang trí
**Chạy:** `python video_khung.py` (cần **ffmpeg/ffprobe** trong PATH; cần `scipy`, `Pillow`, `numpy`)
